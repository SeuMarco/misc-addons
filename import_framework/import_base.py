...
class ImportBase(object):
    ...
    def get_mapping(self):
        """
            @return: { TABLE_NAME : {
                ...
                'map' : { @see mapper
                    'odoo_field_name' : 'external_field_name', or val('external_field_name')
                    'odoo_field_id/id' : ref(TABLE_1, 'external_id_field'), # make the mapping between the external id and the xml on the right
                    'odoo_field2_id/id_parent' : ref(TABLE_1,'external_id_field') # indicate a self dependencies on odoo_field2_id
    ...
    def _fields_mapp(self, dict_sugar, odoo_dict):
        """
...
['', id2, name2] # i=1
        ...
        while True:
            fields = []
            data_lst = []
            for key, val in odoo_dict.items():
    ...
    def xml_id_exist(self, table, external_id):
        """
            Check if the external id exist in the odoo database
